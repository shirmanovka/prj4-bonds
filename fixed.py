...
# Кнопка обновления рендерится до попытки чтения данных, чтобы оставаться
# доступной, даже если текущий bonds.csv повреждён и не парсится —
# иначе пользователь застревает на экране ошибки без способа её исправить.
with st.sidebar:
    common.render_refresh_button(pd.DataFrame(), [load_fixed])
...
try:
    ...
    st.error(
        f"⚠️ Не удалось прочитать `data/bonds.csv`: {e}\n\n"
        "Файл мог повредиться при последнем обновлении. Нажмите «🔄 Обновить "
        "данные» в сайдбаре, чтобы перезаписать его свежими данными."
    )
    st.stop()
