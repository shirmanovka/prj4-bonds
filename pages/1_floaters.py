...
# доступной, даже если текущий floaters.csv повреждён и не парсится.
with st.sidebar:
    common.render_refresh_button(pd.DataFrame(), [load_floaters])

try:
    df_full = load_floaters()
except Exception as e:
    st.error(
        f"⚠️ Не удалось прочитать `data/floaters.csv`: {e}\n\n"
